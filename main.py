...
import cvzone
import os
from utils.identify import ObjectProcessor
...
while cap.isOpened():
    ...
    if result_pose.pose_landmarks:
        mp_drawing.draw_landmarks(frame, result_pose.pose_landmarks, mp_pose.POSE_CONNECTIONS,
                                  mp_drawing.DrawingSpec(color=(200,165, 255), thickness=2),
    ...
    for bb in bbox_id:
        ...
        if fall_ratio > fall_threshold:
            ...
            cv2.putText(frame, "Person Falling", (x1, y1 - 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            cvzone.cornerRect(frame, (x1, y1, width, height), l=20, t=10, colorR=(0, 0, 255))
